## 第7章 表单和验证器[¶](http://web2py.com/books/default/chapter/29/07/forms-and-validators#Forms-and-validators)
...
`grid`和`smartgrid`不会像 crud 那样自动强制执行访问控制，但您可以将其与`auth`使用显式权限检查集成：
