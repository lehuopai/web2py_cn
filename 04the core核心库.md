## 核心[¶](http://web2py.com/books/default/chapter/29/04/the-core#The-core)
...
在第 2 行中，`request=request`是可选的。它具有将当前请求传递到“其他”环境的效果。如果没有这个参数，环境将包含一个新的和空的（除了`request.folder`）请求对象。也可以将响应和会话对象传递给`exec_environment`. 传递请求、响应和会话对象时要小心——被调用动作的修改或被调用动作中的编码依赖关系可能会导致意外的副作用。
